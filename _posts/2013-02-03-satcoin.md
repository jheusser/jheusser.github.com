...
Cryptominisat wins the UNSAT challenge by being faster than all the other solvers. This is interesting as Cryptominisat has been specifically tuned towards cryptographic problems as it is able to detect and treat xor clauses differently to normal clauses [1]. It is extensively used in this case. In the runs above the solver found over 95000 non-binary xor clauses. 
...
However, apart from parameter tuning there's quite a few things to be worked on that should have an even larger impact on performance. Here are a couple of examples:
