...
uses a number of tools used in the program verification domain to attack this problem, see as example [9]. 
Namely, a model checker backed by a SAT solver are used to find the correct nonce or prove the absence of a valid nonce. 
In contrary to brute force which actually executes and computes many hashes my approach is only symbolically executing the hash function with added derived constraints which are inherent in the bitcoin mining process.

The main results beside the recipe for building a SAT-based miner, are: 

1. Simpe parameter tuning of the used tools leads to over 1000% performance improvement.
...
I don't claim that it is a faster approach than brute force, however it is at least theoretically more appealing. 
...
variable, infer values of further variables which logically must follow from the current assignment, and
resolve potential conflicts in the variable assignments by backtracking.
...
A common application of SAT solving is (bounded) model checking, which involves checking whether a system
preserves or violates a given property, such as mutual exclusive access to a specific state in the system.
Model checkers such as [CBMC](http://www.cprover.org/cbmc) [5] directly translate programming languages such as C into CNF formulas, in such a way that
...
the property P (because of the `not`). If it is not possible to make the formula true then the property is guaranteed to holds.
</p>

Most importantly, in case of satisfiability the model checker can reconstruct the variable assignment and 
execution trace (called counterexample) which lead to the violation, using the truth variable assignments provided by the solver.


# Bitcoin mining using SAT Solving and Model Checking

Using the above tools we can attack the bitcoin mining problem in a very different way to brute force.
We take an existing C implementation of sha256 from a mining program and strip away everything but the actual hash function and the basic mining procedure of sha(sha(block)). This C file is going to be the input to CBMC. The aim of this is that with the right assumptions and assertions added to the implementation we direct the SAT solver to find a nonce. Instead of a loop which executes the hash many times and a procedure which checks if we computed a correct hash, we add constraints, which, when satisfied have implicitly the correct nonce in its solution.


<p>The assumptions and assertions can be broken down to the following ideas:
<ul>
<li> The nonce is a modelled as a non-deterministic value 
...
Instead of a loop which continuously increases the nonce, we declare the nonce as a non-deterministic value. 
...
is a way of moving the search from the outside of the actual computation (a brute force loop) to be part of the SAT solver search. 
...
the target (<a href='https://github.com/jgarzik/cpuminer/blob/master/util.c#L396'>see here for an example</a>). We could do the same and just translate this function straight to CNF however there's a much better and more declarative
solution than that in our case: we can just assume values which we know are fixed in the output of the hash. This will restrict the search space to discard any execution paths where the assumptions wouldn't be true anymore. Because we are not in a brute force setting but 
...
{% highlight python %}
  __CPROVER_assume(
     ...
     (state[7]>>8) & 0xff)  == 0x00 && ... );
{% endhighlight %}
where state is the array containing the resulting hash. It might seem unintuitive to "fix" output variables to certain values, however remember that the code is not executed anymore in a regular fashion but translated as a big formula of constraints. Assumptions on the outputs will result in restrictions of the input -- in our case this means only valid nonces will be considered. 
...
The SAT based solution computes only hashes which comply with the mining specification of a valid hash.

## The Assertion 

The most important part is defining the assertion, or the property P as it was called in the section above.
...
And if a satisfiable solution is found, we will get an execution path to a valid nonce value.

In reality, this is encoded more elegantly. As the leading zeros of a hash are already assumed to be true, all that remains to be
asserted is the value of the first non-zero byte in the valid hash will be below the target at that position. Again, we know the position of the 
non-zero byte for sure because of the target. For example, if our current target is the following
...
This program is the input to CBMC, which will encode it to CNF and pass it on to a builtin SAT solver, or alternatively output it as DIMACS to use a different solver. The advantage of using the builtin solver is that, in case of satisfiability, the model checker can easily retrieve a counterexample from the solution which consists of all variable assignments in the solution.

A violation of the assertion implies a hash below the target is found. Let us inspect such a counterexample when run on the [genesis block](https://en.bitcoin.it/wiki/Genesis_block) as input.
...
Cryptominisat wins the UNSAT challenge by being quite a bit faster than all other solvers. This is interesting as Cryptominisat has been specifically tuned towards cryptographic problems, as it is able to detect and treat xor clauses differently to normal clauses [1]. This is extensively used in this case, in the runs above the solver found over 95000 non-binary xor clauses. 

In UNSAT, every solver has to perform the same amount of basic work of trying out 1000 nonce values. The crypto-focused optimisations of cryptominisat could potentially have helped in solving this more efficiently than the remaining solvers.
...
First, it's unsurprising that the SAT timings are generally lower than UNSAT as the solvers don't have to try all possible nonce values. However it is very surprising that ZChaff wins the SAT challenge with quite a good margin to the next solver. ZChaff is the oldest of all solvers presented here, the verison I am using is 9 years old. This could indicate that the heuristics applied by modern SAT solvers do not help in this particular instance. 

Generally, it is not known what makes a SAT instance hard or easy, that leaves only speculation or analysis of the stats provided by the SAT solvers to come to useful conclusions. I could speculate that the avalanche effect of the hash function produces a very structured CNF formula with high dependencies between clauses and variables; perhaps a higher degree of randomisation applied by heuristics performs less well than straight-forward DPLL. I leave this to someone with more SAT solving knowledge to decide. 

Let's try to improve these numbers are bit with some parameter tuning.

## Tuning and Improvements

While the performance numbers are not great (compared to GPU mining) we have to keep in mind that this is entirely unoptimised and there are many ways of how this can be sped up. To give an idea of the performance gains which can be achieved with little effort I am going to use a combination of features: 

- Firstly, the model checker allows to slice the algorithm at the program level (before translating it to CNF). Here, anything that is unrelated to the assertion will be sliced away. As the code for the miner is relatively small and none of the SHA-256 code paths are unused this only slims down the CNF formula by 500 clauses, however it still does have an effect sometimes.
- Secondly, Sat solvers usually have a large number of parameters to play with such as the frequency to pick a random branching choice as decision heuristic. 
...
Simply by adding two parameters to Cryptominisat already improves the performance by over 500%. The restrict parameter is a way to only branch on the 32 most active variables which is intended for cryptography key search -- 32 was picked arbitrarily. In the second row, I tried running it with the plain parameter which deactivates all simplification heuristics, trying to see if the speculations around the ZChaff-speed improvement could also apply to Cryptominisat. Indeed, a 1000% improvement clearly shows that a simpler solver performs better on this instance.

For the last row, I increased the nonce range to 10'000 values which leads to an interesting result. The unoptimised run of this file is 3m39s; this is half the expected time when we take the 42s benchmark on 1000 nonces and assume that the search time increases linearly with the nonce range. This does not seem the case. Apart from that observation, a 600% improvement is achieved with naive parameter tuning.

However apart from parameter tuning there's quite a few things which could be worked on which should have an even larger impact on performance. Here's a couple of examples.

<ul>
<li> It is not using the midstate. This should be the most obvious way of improving performance as with the midstate it will make one less hash call which dramatically reduces the size of the formula (remember there's no function calls but everything is inlined).
<li> As demonstrated, tailored SAT solvers perform better than more off-the-shelf ones. 
Tuning the SAT solver combined with encoding the problem better (see next point) is definitely the way to get better performance. Most solvers have a large number of parameters to tweak. Also, parallel, GPU and FPGA based solvers do exist [8]. 
...
If this is true and is a substantial effect then this is kind of a big deal, because the rate of the money supply is regulated with the difficulty. The higher the difficulty, the less likely it is that a individual block has a valid nonce. However, conventional mining algorithms always have to perform the same amount of work (i.e. try 4 billion nonce values) to reject a block. On the other hand, "Satcoin" miners will get progressively faster which could lead to imbalances in the [controlled supply](https://en.bitcoin.it/wiki/Controlled_supply). These are all just speculations and depend on many factors, first and foremost how well the Sat based approach can be improved and whether the probability of finding a valid nonce does not dwarf the efficiency gain of the algorithm.


To explore this hypothesis, I ran the algorithm also on [block 218430](http://blockexplorer.com/block/000000000000003e3d9fc1d3ffb3a502302797beb27c28a3fbb7000003efd472) which was found end of January 2013. As it is obviously a block later in the history than the genesis block (block 0) its target is smaller. 
The target of the genesis block is the following:
<pre>
00000000ffff0000000000000000000000000000000000000000000000000000
</pre>
While the one of block 218430 is clearly lower and shows more leading zeros which means more of the hash output will be 'fixed' (assumed to be zero) in advance:
<pre>
00000000000005a6b10000000000000000000000000000000000000000000000
</pre>

The runtime to SAT for 1000 nonces again, with the same solvers.
...
Firstly, two more solvers, Minisat and Zchaff, did not terminate within the specified timeout (40 minutes). Apart from that, we see a number of good speed ups compared to the Block 0 timings. Generally, it is hard to benchmark Sat solvers (see [13]) as comparing different heuristics of different solvers can lead to inconsistent results. The paper [13] on benchmarking Sat solver suggest dropping all timeouts and using the geometric mean (in a slightly different context) to evaluate runtimes and speedups. The geometric mean runtime for block 0 over all solvers is 59s, while block 218430 clocks in at 47s, which is indeed faster.

Lastly, let's apply the Zchaff-speed up lesson from the previous section and run the same file with the parameters "plain, restrict=32". The result is 2.5s until SAT which is another 160% speed up to the best time of block 0 in the previous section. Looking at the solver statistics shows less restarts (5 instead of 6), less conflicts (858 vs 158), and less conflict literals (4785 vs 12686) which could indeed hint towards that the proposed algorithm becomes more efficient with higher difficulty.

# Conclusion

I introduced a novel algorithm to solve the bitcoin mining problem without using (explicit) brute force. Instead, the nonce search is encoded as decision problem and solved by a SAT solver in such a way that a satisfiable instance contains a valid nonce. 
The intricate details of encoding a cryptographic hash to CNF is handed off to a bounded model checker such as CBMC. 


SAT solving and bounded model checking have enjoyed many successes in the last decade, however they have never been closer to printing money than this.
